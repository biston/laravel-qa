...
<form id="vote-up-{{ $name }}-{{ $model->id }}" action="/{{ $firstUrl }}/{{ $model->id }}/vote" method="post" style="display: none">
  @csrf
...
<form id="vote-down-{{ $name }}-{{ $model->id }}" action="/{{ $firstUrl }}/{{ $model->id }}/vote" method="post" style="display: none">
  @csrf
  <input type="hidden" name="vote" id="vote" value="-1">
</form>
